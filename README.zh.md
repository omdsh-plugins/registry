...
现在每行都不带 `spec`，所以安装走 `github:<repo>`，会在 `prepare` 里从源码构建。等
`@omdsh-plugins/*` 发布到 npm 之后，把 `build.mjs` 里的 `SPEC_SOURCE` 从 `'github'`
改成 `'npm'` 再重跑：每一行都会带上包名，从 registry 直接安装，不再构建。改一个常量，
所有行一起迁。
