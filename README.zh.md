...
`omdsh-plugins`。所以插件中心装好后不用任何配置就能读到它，
**设置 → 插件 → OMDSH 插件**里立刻就能看到整个集合。
...
- **表达"推荐什么"。** 枚举只能报告"存在什么"。一个草稿仓库和一个已发布的插件，
  在它眼里长得一模一样。
- **钉住版本。** 插件中心靠比较"源声明的版本"和"磁盘上的版本"来决定要不要提示更新。
...
插件在面板里往上挪，要改的是它自己 `package.json` 里的 `order`，然后重新生成，
而不是挪这个文件里的某一行。

一行写坏只损失这一行——插件中心会丢掉它，清单其余部分照常生效。同一个包名出现两次
则损失后出现的那一行：先出现的赢。这样就算写重复了，目录里有哪个包也不取决于
那处错误恰好落在文件的哪个位置。
...
远程清单里的路径，意思就是"装读者机器上那个位置碰巧有的东西"。

## 它是生成的，不是手写的

`build.mjs` 读取集合 checkout 里与本目录并列的插件仓库，从它们的 `package.json`
写出清单。上面每个字段本来就住在那里，要是手抄一份，有人改一句简介它就会开始漂移。

哪些包会出现，由包自己决定：一个并列目录声明了 `dsh.bundle.patch` 才会被收进来，
插件中心判断一个包"可安装"，靠的也是同一个事实。集合里两个应用工作区——`omdsh-desktop`
...
根本没走到那么深去读它。反正它本来也不该被列出来——一个 profile 只组装一个界面，
它注册的 id 又与 web 应用冲突，所以它是靠搭一个 TUI profile 装进去的，不是往运行中的
...
谁是哪种，由 `build.mjs` 顶部的 `ON_NPM` 集合决定。**里面是插件中心和模式系统。**
插件中心是引导的那一环——必须从 npm 装，机器才不用先 clone 任何东西就能拿到安装器。
模式系统也已经发布，两个模式插件按名字装它，所以再走 git 安装，就是给一个本来已经带
`lib/` 的包白跑一遍 `prepare`。其余每个插件都从 GitHub 装，哪怕它也在 npm 上。如果
`npm publish` 一成功就把名字加进去，安装按钮就会去拉 registry 上的那一份——但这不是
这套集合装它的方式。

目前 `@omdsh-plugins/omdsh-plughub` 和 `@omdsh-plugins/omdsh-basemode` 从 npm 安装。
`node registry/build.mjs` 会把这个划分打印出来，所以谁要是把别的名字划到了 npm 那边，
重跑一次当场就看得见，不用等一周后有人点了那张卡片才发现。
...
个插件，要么以非零状态退出并说明文件已过期。不带参数的那条才是重写 `registry.json` 的。
...
- **生成脚本只认自己的账号。** 一个包的 `repository.url` 指向 `omdsh-plugins`
  以外的账号时，会被警告并跳过——指向别人账号的一行，轮不到这个 upstream 来推荐。
  所以 fork 要是还没改写 `repository` 字段，重新生成出来的清单就会悄悄变短，
  每个包只留下一条警告。
